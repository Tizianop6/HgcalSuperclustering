...
OutputDir = "/eos/user/t/tipaulet/www/PtResolutionCPComparison/"
    ...
    for i in range(len(dumperInputV5.inputReaders)):
        ...
        for ev in range(len(tms)):
            ...
            for simCand_idx in range(len(simCandEv.simTICLCandidate_raw_energy)):
                ...
                if sharedE[ak.argmin(score)]/simRegrEnergy < 0.3: continue
                ...
                #Check if right track associated
                if track_in_cand != simTrack: continue

                #print("selection:",time.process_time() - start)
                pt_trackster=tmEv.raw_pt[tid]
                pt_track=tracksEv.track_pt[simTrack]#*math.cosh(eta)
                pt_merged=(pt_trackster+pt_track)/2

                pt_ts_list.append(pt_trackster)
                pt_trck_list.append(pt_track)
                pt_merged_list.append(pt_merged)
