...
def FindCandidate(tracksters_in_cand,track_in_cand,tid):
    cand_idx = -1
            #print(candEv.tracksters_in_candidate)
...
def AnalyzeFile(dumper, cands, simCands, ass,dict_singlefiles,counter):
    ...
    for ev in range(len(cands)):
        ...
        #tracksEv = tracks[ev]
        #print(tracksEv)
        #print("len(simCandEv.simTICLCandidate_raw_energy)",len(simCandEv.simTICLCandidate_raw_energy))
        for simCand_idx in range(2):#len(simCandEv.simTICLCandidate_raw_energy)):
            ...
            if(t_MTD or t_HGCal):
                ...
                dict_temp["cp_energy"].append(simRegrEnergy)
                if (t_HGCal==False):
...
def ReadFileAndFit(filename,key,tag="prova",eta=2.2, maxfiles=1):
    ...
    # Read input folder
    dumperInputV5 = DumperInputManager([filename], limitFileCount=maxfiles)
    ...
    for i in range(len(dumperInputV5.inputReaders)):
        ...
        p1 = multiprocessing.Process(target=AnalyzeFile, args=(dumper, cands, simCands, ass, dict_singlefiles,i))
        jobs1.append(p1)
        p1.start()
    ...
    with open('files_onlyPV/residuals_'+str(key)+'_'+str(tag)+'.pkl', 'wb') as handle:
