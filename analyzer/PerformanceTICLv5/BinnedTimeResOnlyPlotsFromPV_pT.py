...
OutputDir = "/eos/user/t/tipaulet/www/BinnedTimeResPUOnlyPVPt/"
...
def CreatePlots(dict_residuals,eta):
    ...
    plt.savefig(OutputDir +"eta"+str(eta)+"mpl_woerr.pdf")
...
if __name__ == "__main__":
    ...
    for i in tqdm(range(len(folders_2p2))):
        ...
        with open('files_onlyPV_Pt/residuals_'+match.group(1)+'_'+folders_2p2[i]+'.pkl', 'rb') as file:
    ...
    for i in tqdm(range(len(folders_1p9))):
        ...
        #dict_residuals.update(ReadFileAndFit(file_name,return_dict,int(match.group(1)),folders_2p2[i],eta,maxfiles))
        #print(dict_residuals)
        with open('files_onlyPV_Pt/residuals_'+match.group(1)+'_'+folders_1p9[i]+'.pkl', 'rb') as file:
            data=pickle.load(file)
        #print(data)
